...
def _initialize_ryp() -> None:
    ...
    if _rlib is None:
        with _init_lock:
            if _rlib is None:
                # Set _ryp_PID to the initializing process's process ID
...
def r(R_code: str = ...) -> None:
    ...
    # Initialize ryp, if not already initialized
    _initialize_ryp()
    # Lock so only one thread at a time can use the R interpreter
    with _R_lock:
        if R_code is ...:
            ...
            error_message = (
                f'R_code has type {type(R_code).__name__!r}, but must be a '
                f'string')
            raise TypeError(error_message)
        # Check that R_code does not contain null bytes, to avoid
        # Rf_mkCharLenCE() raising an error when converting it to a character
        # vector later
        if '\0' in R_code:
        ...
        # line should be printed, like R's own REPL does in Rf_ReplIteration,
        # but R_Visible is no longer exported. The limitations of R's API in
        # this regard are discussed at:
        ...
        try:
            ...
            # - Wrap in R_tryCatchError() to avoid crashing Python in certain
            #   edge cases where parsing actually raises an R error instead of
            #   setting status to PARSE_INCOMPLETE, PARSE_ERROR, or PARSE_EOF,
            #   for instance when R_code contains an invalid escape sequence
            #   like \S.
            # - Because the withAutoprint() may affect the parse error, reparse
            #   the code without it, just to get the correct error message.
            #   (This isn't necessary in nested mode, since we didn't wrap in
            #   withAutoprint()). The reparse shouldn't raise an error if the
            #   original parse didn't, so there's no need to wrap the reparse in
            #   R_tryCatchError().
            status = _ffi.new('int[1]')
            e = None
            ...
            def onerror(exception, exc_value, traceback):
                ...
                # ffi.callback; instead, store them in a nonlocal and raise
                # after
                nonlocal e
                e = exception
            ...
            if status[0] == 0:
                ...
                if not nested:
                    _rlib.R_ParseVector(
                        ...
                        -1, status, _rlib.R_NilValue)  # reparse w/o autoprint
                parse_error_message = \
                    _ffi.string(_rlib.R_ParseErrorMsg).decode('utf-8')
                # The code snippet that triggered the syntax error is stored in
                # a circular buffer that stretches back from
                # `R_ParseContextLast` to the first null byte, wrapping around
                # to the end of the buffer if the beginning of the buffer is
                # reached before finding a null byte
                ...
                if '\n' in parse_context:
                    ...
                    error_message = \
                        f'{parse_error_message} in "{parse_context}"'
                raise SyntaxError(error_message)
            # Evaluate the parsed expression; it's always a single expression,
            # due to the withAutoprint(). (Except in nested mode, in which case
            # raise an error if it's multiple expressions.) Use
            # R_tryEvalSilent() rather than Rf_eval() so we can check if it ran
            # OK via the status argument.
            ...
            if nested:
                ...
                if num_statements > 1:
                    error_message = (
                        f'R_statement contains {num_statements:,} R '
                        f'statements, but must contain a single statement')
                    raise ValueError(error_message)
            ...
            # Also, remove the 'Error in eval(ei, envir) : ' prefix
            # withAutoprint() induces when the error occurs at the top level,
            # the 'Error: ' prefix that evaluating a single statement from
            # to_py() induces, and the 'Error ' prefix that occurs when the
            # error occurs in a function. (The 'Error: ' prefix also happens at
            ...
            # (R_Warnings and PrintWarnings() are not exposed, so this is a
            # hacky workaround.) Note that this resets error_buffer, which is
            # why we get the error message from evaluating the parsed
            # expression first.
            ...
            # thread to handle plot events, or run on the main thread on
            # Windows and Mac
            ...
            if not _jupyter_notebook:
                if platform.system() == 'Windows':
                    ...
                    if (_plot_event_thread is None or not
                            _plot_event_thread.is_alive()) and \
                            _plot_window_open():
...
def to_r(python_object: Any, R_variable_name: str, *,
    ...
    with _R_lock:
        ...
        # indicates it's a recursive call, it allows descriptive variable names
        # in error messages, and it stops protected objects from being
        # garbage-collected until the top-level to_r() call returns. When
        # calling to_r() recursively, return the converted R object rather than
        # assigning it to the R global namespace. (Among other advantages, this
        # simplifies stack traces relative to calling a second function when
        # recursing.)
        if isinstance(R_variable_name, str):
            ...
            if format not in ('matrix', 'data.frame', 'keep', None):
                if isinstance(format, str):
                    ...
                    error_message = (
                        f"format has type {type(format).__name__!r}, but must "
                        f"be 'matrix', 'data.frame', 'keep', or None")
                    raise TypeError(error_message)
            python_object_name = 'python_object'
            rmemory = _RMemory(_rlib)
            top_level = True
        elif isinstance(R_variable_name, tuple) and len(R_variable_name) == 2 \
                and isinstance(R_variable_name[0], str) and \
        ...
        # NumPy array) or something that might contain one (list, tuple, or
        # dict)
        if raise_format_errors and top_level and \
            ...
            if is_ndarray:
                error_message = (
                    f'python_object is a {python_object.ndim:,}D NumPy array, '
                    f'not a 2D one, so format must be None')
                raise TypeError(error_message)
            else:
                error_message = (
                    ...
                    f'MultiIndexes.)')
                raise TypeError(error_message)
        # If format is None, default to what's in the config (or False for
        # NumPy)
        if format is None:
        ...
        try:
            ...
            if rownames is not None and top_level:
                if isinstance(python_object, (bytes, bytearray)):
                    error_message = (
                        ...
                        f'{type(python_object).__name__!r}, but raw vectors '
                        f'lack rownames, so rownames must be None')
                    raise TypeError(error_message)
                elif is_pandas and (is_df or is_series):
                    error_message = (
                        f'python_object is a pandas '
                        f'{"DataFrame" if is_df else "Series"}, so rownames '
                        f'must be None; specify rownames via '
                        f'python_object.index instead')
                    raise TypeError(error_message)
            ...
            # but a multidimensional NumPy array, scipy sparse array or matrix,
            # or something that might contain one (list, tuple, or dict)
            if colnames is not None and top_level:
                if not is_multidimensional_ndarray and not is_sparse and not \
                    ...
                    error_message = (
                        f'python_object is not a multidimensional NumPy array '
                        f'or scipy sparse array or matrix, so colnames must be '
                        f'None. It has type {type(python_object).__name__!r}.\n'
                        f'(colnames can also be specified when python_object '
                        f'is a list, tuple, or dict, since these may contain '
            ...
            if python_object is None:
                ...
                if abs(python_object) > 2_147_483_647:
                    ...
                    result = \
                        rmemory.protect(_rlib.Rf_ScalarInteger(python_object))
                ...
                if '\0' in python_object:
                    error_message = (
                        r'python_object is a string that contains a null byte '
                        r'(\0)')
                    raise SyntaxError(error_message)
                result = _string_to_character_vector(python_object, rmemory)
            elif isinstance(python_object, (bytes, bytearray)):
                result = rmemory.protect(
                    _rlib.Rf_allocVector(_rlib.RAWSXP, len(python_object)))
                ...
                for key in python_object:
                    if not isinstance(key, str):
                        error_message = (
                            ...
                            f'key {key!r}, which has type '
                            f'{type(key).__name__!r} rather than str and thus '
                            f'cannot be converted into the name of an R list '
                            f'element')
                        raise TypeError(error_message)
                result = rmemory.protect(
                    _rlib.Rf_allocVector(_rlib.VECSXP, len(python_object)))
                ...
                if is_df:
                    # If format='matrix', all columns must have the same data
                    # type, and the data type cannot be Date or Datetime;
                    ...
                    if format == 'matrix':
                        if len(unique_dtypes) > 1:
                            error_message = (
                                ...
                                f"with a mix of data types (i.e. "
                                f"len(set({python_object_name}.dtypes)) > 1) "
                                f"so format='matrix' cannot be specified")
                            raise TypeError(error_message)
                        ...
                        if dtype == pl.Date or dtype == pl.Datetime:
                            ...
                            error_message = (
                                f"{python_object_name} is a polars DataFrame "
                                f"where all columns have data type {dtype!r}, "
                                f"so format='matrix' cannot be specified.\n"
                                f"({R_type} matrices are not supported, only "
                        ...
                        if len(bad_cols) > 0:
                            ...
                            error_message = (
                                f'{python_object_name}[{bad_col!r}] is a '
                                f'polars Series with data type '
                        ...
                        # we handle zero-copy as Series) or pl.Object, fall back
                        # to converting to dict, converting each column to R
                        ...
                        if pl.UInt32 in unique_dtypes or \
                            ...
                            result = to_r(
                                ...
                                # deliberately leave python_object_name as-is
                                # (if column 'a' has an error, the error message
                                # will correctly say python_object['a'])
                        ...
                        # Otherwise, convert to Arrow
                            ...
                            arrow = python_object.rechunk().to_arrow()\
                                .to_batches()[0]
                        else:
                            # python_object.to_arrow().to_batches() == [], so
                            # do:
                    ...
                    if any(dtype == bad_dtype for bad_dtype in bad_dtypes):
                        error_message = (
                            ...
                            f'data type {dtype.base_type()!r}, which lacks an '
                            f'R equivalent')
                        if dtype == pl.Decimal:
                            error_message += (
                                '.\nConsider converting to Float64 before '
                    ...
                    # UInt64/Int64 as double, to allow zero-copy. For
                    # UInt64/Int64, we will reinterpret the double array as
                    # bit64::integer64 on the R side by manually adding
                    # 'integer64' as a class.
                    elif dtype == pl.UInt32 and \
                ...
                if is_df or is_series:
                    ...
                    if isinstance(python_object.index, pd.MultiIndex):
                        error_message = (
                            f'{python_object_name}.index is a pandas '
                            f'MultiIndex, which lacks an R equivalent.\nUse '
                            f'reset_index() to retain the index as a column, '
                    ...
                    # categorical with string categories, since R rownames must
                    # be strings. Assume dtype = object is a string index for
                    # now; it will fail to convert to R if not all elements are
                    # strings. Convert length-0 object indices to StringDtype so
                    # that they convert to character(0) rather than
                    # vctrs::unspecified(0).
                    ...
                    if pd.api.types.is_string_dtype(index.dtype) or \
                        ...
                        error_message = (
                            ...
                            f'{python_object_name}.index is neither the '
                            f'default index '
                            f'(pd.RangeIndex(len({python_object_name}))) nor a '
                            f'string or string categorical Index, but R '
                            f'rownames must be strings.\nBefore calling '
                            f'to_r(), call reset_index() to retain the index '
                            f'as a column, or reset_index(drop=True) to '
                            f'discard the index.')
                        raise TypeError(error_message)
                    ...
                    if is_df and python_object.shape[1] > 0:
                        if not (pd.api.types.is_string_dtype(
                                ...
                                pd.api.types.is_string_dtype(
                                    python_object.columns.dtype.categories
                                    .dtype)):
                            error_message = (
                                f'{python_object_name} is a pandas DataFrame '
                                f'where {python_object_name}.columns is not a '
                                f'string or string categorical Index (it has '
                                f'data type {python_object.columns.dtype!r}), '
                                f'but R colnames must be strings')
                            raise TypeError(error_message)
                ...
                if is_df and isinstance(index, pd.RangeIndex) and \
                        index.start == 0 and \
                        index.stop == len(python_object) and index.step == 1:
                    # If format='matrix', all columns must have the same data
                    # type, which cannot be datetime64, PeriodDtype or
                    # ArrowDtype(pa.timestamp()); concatenate the columns into a
                    # single Series
                    ...
                    if format == 'matrix':
                        ...
                        if ((pa.types.is_timestamp(dtype.pyarrow_dtype) or
                            ...
                            dtype = str(dtype) if is_arrowdtype else \
                                'PeriodDtype' if is_perioddtype else \
                                'datetime64'
                            error_message = (
                                f"{python_object_name} is a pandas DataFrame "
                        ...
                        # columns, as well as pd.ArrowDtype if its pyarrow dtype
                        # is unsupported
                        for dtype in unique_dtypes:
                            ...
                            if dtype.startswith('|S'):
                                error_message = (
                                    f'{python_object_name} is a pandas '
                                    f'DataFrame that contains one or more '
                                    f'columns with the bytestring data type '
                                    f'{dtype!r}, which lacks an R equivalent.\n'
                                    f'Consider converting these columns to a '
                        ...
                        if any(isinstance(dtype, pd.IntervalDtype)
                            ...
                            error_message = (
                                f'{python_object_name} is a pandas DataFrame '
                                f'that contains one or more Interval columns '
                        ...
                        if any(isinstance(dtype, pd.SparseDtype)
                            ...
                            error_message = (
                                f'{python_object_name} is a pandas DataFrame '
                                f'that contains one or more Sparse columns '
                        ...
                        for dtype in unique_dtypes:
                            if isinstance(dtype, pd.ArrowDtype) and not \
                                ...
                                error_message = (
                                    f'{python_object_name} is a pandas '
                                    f'DataFrame that contains one or more '
                                    f'columns with the pyarrow data type '
                                    f'{str(dtype.pyarrow_dtype)!r}, which '
                                    f'lacks an R equivalent.\n(Only integer, '
                                    f'floating-point, temporal, string, '
                                    f'large_string, dictionary<values=string>, '
                                    f'and null pyarrow data types are '
                                    f'supported.)')
                                raise TypeError(error_message)
                        ...
                        if any(isinstance(dtype, pd.CategoricalDtype) and not
                            ...
                            error_message = (
                                f'{python_object_name} is a pandas DataFrame '
                                f'that contains one or more categorical '
                                f'columns with non-string categories, which '
                                f'lack an R equivalent')
                            raise TypeError(error_message)
                        # Convert float16/float128 (which Arrow does not
                        # support) to float. Use dtype == 'float128', not
                        # dtype == np.float128, since np.float128 isn't defined
                        # on Windows.
                        ...
                        # R separately, then converting to data.frame
                        ...
                        # Otherwise, convert to Arrow
                            ...
                            if any(isinstance(dtype, pd.ArrowDtype) and
                                ...
                                for col, dtype in python_object.dtypes.items():
                                    ...
                                    assigns[col] = pd.Series(
                                        pa.array(python_object[col])
                                        .cast(new_dtype),
                                        index=python_object.index,
                ...
                # If python_object is a Series with the default index (or for
                # which we've already handled the index), or an
                # Index/DatetimeIndex...
                    ...
                    # as well as pd.ArrowDtype if its pyarrow dtype is
                    # unsupported
                    type_name = 'Series' if is_series else 'Index'
                    if str(dtype).startswith('|S'):
                        error_message = (
                            f'{python_object_name} is a pandas {type_name} '
                            f'with the bytestring data type {str(dtype)!r}, '
                            f'which lacks an R equivalent.\nConsider '
                            f'converting to a string data type before calling '
                            f'to_r().')
                        raise TypeError(error_message)
                    if isinstance(dtype, pd.IntervalDtype):
                        error_message = (
                            f'{python_object_name} is a pandas {type_name} '
                            f'with Interval data type (pd.IntervalDtype), '
                            f'which lacks an R equivalent')
                        raise TypeError(error_message)
                    if isinstance(dtype, pd.SparseDtype):
                        error_message = (
                            f'{python_object_name} is a pandas {type_name} '
                            f'with Sparse data type (pd.SparseDtype), which '
                            f'lacks an R equivalent')
                        raise TypeError(error_message)
                    is_arrowdtype = isinstance(dtype, pd.ArrowDtype)
                    if is_arrowdtype and not \
                        ...
                        error_message = (
                            f'{python_object_name} is a pandas {type_name} '
                            f'with the pyarrow data type '
                            f'{str(dtype.pyarrow_dtype)!r}, which lacks an R '
                            f'equivalent.\n(Only integer, floating-point, '
                            f'temporal, string, large_string, '
                    ...
                    if isinstance(dtype, pd.CategoricalDtype) and not \
                        ...
                        error_message = (
                            ...
                            f'{type_name} with non-string categories, which '
                            f'lacks an R equivalent')
                        raise TypeError(error_message)
                    ...
                    # Otherwise, convert to Arrow
                        ...
                        if len(python_object) == 0 and is_arrowdtype and \
                            ...
                            if dtype == object:
                                ...
                                # View uint32 as int32 and uint64/int64 as
                                # double, to allow zero-copy. For uint64/int64,
                                # we will reinterpret the double array as
                                # bit64::integer64 on the R side by manually
                                # adding 'integer64' as a class.
                                if dtype == np.uint32:
                                    arrow = pa.array(
                                    ...
                                    # Match the unsigned behavior in the
                                    # zero-copy case by casting pa.uint32 to
                                    # pa.int32 and pa.uint64 to pa.int64
                                    arrow = python_object.array._pa_array
                                    if pa.types.is_uint32(dtype.pyarrow_dtype):
                                        arrow = arrow\
                                            .cast(pa.int32(), safe=False)
                                    elif pa.types.is_uint64(
                                            dtype.pyarrow_dtype):
                                        arrow = arrow\
                                            .cast(pa.int64(), safe=False)
                                else:
                                    # Match the unsigned behavior in the
                                    # zero-copy case by casting UInt32Dtype to
                                    # Int32Dtype and UInt64Dtype to Int64Dtype
                            ...
                            # pa.ChunkedArray instead of a pa.Array (which
                            # happens when converting an ArrowDtype DataFrame to
                            # a matrix, due to the pd.concat)
                    ...
                    result = \
                        rmemory.protect(_rlib.Rf_allocVector(_rlib.LGLSXP, 1))
                ...
                if is_ndarray and python_object.ndim > 0 and \
                    ...
                    error_message = (
                        f'{python_object_name} is a {python_object.ndim:,}D '
                        f'NumPy array with length {max(python_object.shape):,} '
                        f'along dimension {max_dimension:,}, more than '
                        f'INT32_MAX (2,147,483,647), the maximum supported in '
                        f'R')
                    raise ValueError(error_message)
                if python_object.ndim == 0:  # generic or 0D ndarray
                    ...
                    if is_nat:
                        ...
                        if dtype == 'float128':
                            ...
                            if is_datetime64 or is_timedelta64:
                                if dtype.name[-3:-1] == 'ns' or dtype.name[-2] \
                                        in ('DWMY' if is_datetime64 else 'MY'):
                    ...
                    # Disallow bytestring and structured array (np.void) dtypes
                    # and zero-length datetime64 and timedelta64 arrays with no
                    # time unit, and multidimensional datetime64 arrays (or
                    # dtype=object arrays of datetimes)
                    ...
                    if dtype == np.complex64 or dtype == np.complex128:
                        ...
                        # View uint32 as int32 and uint64/int64 as double, to
                        # allow zero-copy. For uint64/int64, we will reinterpret
                        # the double array as bit64::integer64 on the R side by
            ...
            # Only support arrow Arrays when recursing, not at the top level
                ...
                if python_object.nnz > 2_147_483_647:
                    error_message = (
                        ...
                        f'with {python_object.nnz:,} non-zero elements, more '
                        f'than INT32_MAX (2,147,483,647), the maximum '
                        f'supported in R')
                    raise ValueError(error_message)
                ...
                _rlib.R_do_slot_assign(
                    result, _rlib.Rf_install(b'factors'),
                    rmemory.protect(_rlib.Rf_allocVector(_rlib.VECSXP, 0)))
            ...
            # Let Arrow handle conversions of temporal types
                ...
                if isinstance(python_object, datetime.time) and \
                    ...
                    error_message = (
                        f'{python_object_name} is a datetime.time object with '
                        f'a non-missing time zone, which cannot be represented '
                        f'in R')
                    raise TypeError(error_message)
                result = to_r(pa.array([python_object]), (
                    f'pyarrow.array([{python_object_name}])', rmemory))
            ...
            if arrow is not None:
                if is_polars:
                    # ...for DictionaryArrays (Categoricals and Enums), work
                    # around github.com/apache/arrow/issues/39603 by casting the
                    # index type from uint32 to int32, work around
                    # github.com/pola-rs/polars/issues/14709 by casting Enums to
                    # ordered (not unordered) DictionaryArrays, and stop R
                    # warning about "Coercing dictionary values to R character
                    # factor levels" by casting large_string to string (and also
                    # work around github.com/apache/arrow/issues/40128 when
                    # arrow has a length of 0)...
                ...
                # arrow::Array/RecordBatch$import_from_c
                ...
                Array_or_RecordBatch = _rlib.Rf_eval(_rlib.Rf_findVarInFrame(
                    arrow_namespace, _rlib.Rf_install(
                        b'RecordBatch' if is_df else b'Array')),
                    _rlib.R_BaseEnv)
                function_call = rmemory.protect(_rlib.Rf_lang3(
                ...
                # the R Arrow Array to a vector (using array$as_vector() instead
                # of as.vector(array) to work around
            ...
            # but format == 'matrix' and we did not convert the index
            # separately, convert to a matrix/array by adding dims and (if
            # specified) rownames and colnames
            if is_multidimensional_ndarray or is_df and format == 'matrix' and \
                    not converted_index_separately:
                # For difftime and vctrs::unspecified, manually add 'array' and
                # (if 2D) 'matrix' as classes (R's array()/matrix() functions do
                # this too, but also overwrite any existing clases, like
                # difftime)
                if _rlib.Rf_inherits(result, b'difftime') or \
                    ...
                    for i, class_name in enumerate(classes_to_add,
                        ...
                        _rlib.SET_STRING_ELT(
                            new_classes, i, _rlib.Rf_mkCharLenCE(
                    ...
                    if isinstance(python_object, (int, float, str, complex)):
                        if rowname_length != 1:
                            error_message = (
                                ...
                                f'{python_object_name} is a scalar, '
                                f'specifically of type '
                                f'{type(python_object).__name__!r}, so '
            ...
            if add_integer64:
                ...
                _rlib.Rf_setAttrib(result, _rlib.R_ClassSymbol, new_classes)
            # If converting 2D NumPy array -> data.frame, convert result (which
            # is now an R matrix/array) to a data.frame with as.data.frame()
...
def to_py(R_statement: str, *,
    ...
    with _R_lock:
        ...
        with _RMemory(_rlib) as rmemory:
            # Defer loading Arrow until calling to_py() or to_r() for speed.
            # Allow R_statement to be a tuple of (R_statement, R_object) instead
            # of a string. This is used when recursively calling to_py(). In
            # this case, don't check inputs for validity.
            with ignore_sigint():
                import pyarrow as pa
            from pyarrow.cffi import ffi as pyarrow_ffi
            if isinstance(R_statement, str):
                ...
                # object it names; otherwise, evaluate it with r() and return
                # the object it evaluates to
                if _is_valid_R_variable_name(R_statement):
                    ...
                    # allow R_code to be a tuple of (str, RMemory), where str is
                    # the string of R code. This serves two purposes: it
                    # indicates it's a nested call, and it stops the object that
                    # results from evaluating R_code from being
                    # garbage-collected until to_py() returns.
            ...
            # See adv-r.hadley.nz/base-types.html for an overview of R's C
            # types, and lines 110-142 of
            ...
            if R_object_type == _rlib.NILSXP:  # NULL
                ...
                if raise_squeeze_errors:
                    error_message = (
                        ...
                        f'{"data.frame" if is_df else "list"}, so squeeze must '
                        f'be None')
                    raise TypeError(error_message)
                if names == _rlib.R_NilValue:
                    if is_df:  # zero-column data.frames may lack names
                        ...
                        try:
                            ...
                            error_message = (
                                ...
                                f'converting the list {R_statement!r} to '
                                f'Python; this usually means the list contains '
                    ...
                    try:
                        ...
                        # be '' if some list elements have names and some don't
                        # (in which case we use the 0-based integer index as the
                        # dict key), but also if R_object is a data.frame with a
                        # column named '' (in which case we use '' as the dict
                        # key)
                ...
                if index:
                    if index in result:
                        error_message = (
                            ...
                            f'the name that was going to be used for the index '
                            f'column.\nSet index to a different string, or set '
                            f'index=False to not convert the index.')
                        raise ValueError(error_message)
                    rownames = \
                ...
                if format == 'polars':
                    try:
                        ...
                        error_message = (
                            ...
                            "options(to_py_format='pandas') to change the "
                            "default format")
                ...
                if raise_format_errors:
                    error_message = (
                        f'{R_statement!r} is a raw vector, which always '
                ...
                if raise_squeeze_errors:
                    error_message = (
                        f'{R_statement!r} is an S4 object, so squeeze must be '
                        f'None')
                    raise TypeError(error_message)
                ...
                slot_names = _call(function_call, rmemory,
                                   f'cannot convert {R_statement!r} to an '
                                   f'arrow Array')
                # Recurse over them
                result = {}
            ...
            if input_type == 'array':
                if raise_format_errors:
                    error_message = (
                        f'{R_statement!r} is a {len(shape):,}D array, which '
                        f'can only be converted into a NumPy array, so format '
                        f'must be None')
                    raise TypeError(error_message)
                if index is not None:
                    error_message = (
                        f'{R_statement!r} is a {len(shape):,}D array, which '
                        f'can only be converted into a NumPy array, so index '
                        f'must be None')
                    raise TypeError(error_message)
                format = 'numpy'
                index = None
            ...
            if R_object_type != _rlib.CPLXSXP:
                ...
                # This code should be updated to use the capsule interface
                # (arrow.apache.org/docs/format/CDataInterface/
                # PyCapsuleInterface.html) once its docs are updated
                # (github.com/apache/arrow/issues/39198). See
                # github.com/rpy2/rpy2-arrow/blob/main/rpy2_arrow/arrow.py for a
                ...
                if R_arrow_array is None:
                    ...
                    if R_object_type == _rlib.REALSXP and \
                        ...
                        # Use arrow::as_arrow_array(R_object,
                        # type=arrow::int64()) for bit64::integer64 arrays.
                        # Arrow is not fooled by our hack in to_r() of
                        # reinterpreting int64/uint64 arrays as double and then
                        # adding 'integer64' as a class in to_r(), and will
                        # convert these arrays to a DoubleArray with
                        # arrow::Array$create().
                        as_arrow_array = _rlib.Rf_findVarInFrame(
                            arrow_namespace,
                            _rlib.Rf_install(b'as_arrow_array'))
                        ...
                        R_arrow_array = _call(function_call, rmemory,
                                              f'cannot convert {R_statement!r} '
                                              f'to an arrow int64 Array')
                    elif R_object_type == _rlib.REALSXP and \
                            _rlib.Rf_inherits(R_object, b'difftime'):
                        # Use arrow::as_arrow_array(R_object,
                        # type=arrow::duration('ns')) for timedeltas (i.e.
                        # difftimes without the hms class) and
                        # arrow::as_arrow_array(R_object,
                        # type=arrow::time64('ns')) for times (which have both
                        # the difftime and hms classes) to work around
                        # github.com/apache/arrow/issues/40109.
                        as_arrow_array = _rlib.Rf_findVarInFrame(
                            arrow_namespace,
                            _rlib.Rf_install(b'as_arrow_array'))
                        hms = _rlib.Rf_inherits(R_object, b'hms')
                        duration_or_time64_name = \
                            b'time64' if hms else b'duration'
                ...
                # environments, so the method call is identical to a function
                # call, but with the environment set to R_arrow_array, not
                # R_BaseEnv.
                array = pyarrow_ffi.new('struct ArrowArray*')
                schema = pyarrow_ffi.new('struct ArrowSchema*')
                array_ptr = int(pyarrow_ffi.cast('uintptr_t', array))
                ...
                # If R_object is a scalar and squeeze=True, return a Python
                # scalar
                ...
                # NumPy and pandas have a complex dtype with the same memory
                # layout (a pair of float64s), so just reinterpret the memory.
                # polars doesn't, so leave it up to the user what to do.
                if format == 'polars':
                    error_message = (
                        f"{R_statement!r} is a complex {input_type}, which is "
                        f"not supported by polars.\nConsider either switching "
                        f"to format='numpy', format='pandas' or "
                        f"format='pandas-pyarrow', or splitting the "
                        f"{input_type} into its real and imaginary parts with "
                        f"Re({R_statement}) and Im({R_statement}) and "
                        f"converting them separately.")
                    raise TypeError(error_message)
                # Need to copy() to avoid sharing memory with R, which is
                # dangerous without Arrow's mechanism for lifetime management
            ...
            if squeeze and _rlib.Rf_xlength(R_object) == 1:
                ...
                if index is not None:
                    ...
                    raise TypeError(error_message)
                result = result[0]
                # pyarrow converts DurationScalar[ns] to pd.Timedelta rather
                # than datetime.timedelta like for DurationScalar[us] and above,
                # because datetime.timedelta can't represent ns. Instead, just
                # truncate to the nearest microsecond, then convert to
                # datetime.timedelta.
            ...
            if index or input_type == 'matrix' and format != 'numpy':
                ...
                if input_type == 'vector':
                    ...
                    dimnames = \
                        _rlib.Rf_getAttrib(R_object, _rlib.R_DimNamesSymbol)
                ...
                if rownames != _rlib.R_NilValue:
                    ...
                    rownames = to_py((f'{rownames_name}({R_statement})',
                                      rownames),
                                     format=format, index=False, squeeze=False)
                else:
                    index = False
                ...
                if R_object_type != _rlib.CPLXSXP:
                    if result.null_count == 0 and (
                        ...
                        result = result.to_numpy(zero_copy_only=False,
                                                 writable=True)
                    # If R_object is a matrix, reshape only once result is a
                    # NumPy array since Arrow doesn't have a notion of
                    # memory-contiguous 2D arrays
                    if multidimensional:
                        result = result.reshape(shape, order='F')
            else:
                if multidimensional:
                    # R_object is specifically a matrix here (i.e. 2D), not a
                    # >2D array, since we set format='numpy' for >2D arrays
                    # above
                    nrows, ncols = shape
                    ...
                    if index and index in colnames:
                        error_message = (
                            ...
                            f'index to a different string, or set index=False '
                    ...
                    if index == vector_name:
                        error_message = (
                            f'the converted vector {R_statement!r} has the '
                            f'same name as the index.\nSet index to a '
                            f'different string, or set index=False to not '
                            f'convert the index.')
                        raise ValueError(error_message)
                if format == 'polars':
                    try:
                    ...
                    # levels(R_object)
                    if (result.shape[1] > 0 and
                            result.dtypes[0] == pl.Categorical
                            if multidimensional else
                            result.dtype == pl.Categorical) and \
                            _rlib.LOGICAL_ELT(
                    ...
                    if index:
                        ...
                        if result.width:
                            ...
                            # polars forces all 0-column DataFrames to have
                            # length 0, so manually make a DataFrame of the
                            # correct length
